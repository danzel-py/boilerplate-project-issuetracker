...
suite('Functional Tests', function() {
  ...
  test('Update one field on an issue: PUT request to /api/issues/{project}', () => {
    ...
    chai.request(server)
      ...
      .end((err, res) => {
        assert.equal(err, null)
        assert.deepEqual(res.body, { "result": "successfully updated", "_id": "605ea97e3eb65d084a8ea5cc" })
      })
  })
  ...
  test('Update an issue with no fields to update: PUT request to /api/issues/{project}', () => {
    chai.request(server)
      ...
      .end((err, res) => {
        assert.equal(err, null)
        assert.deepEqual(res.body, { error: 'no update field(s) sent', '_id': '605ea97e3eb65d084a8ea5cc' })
      })
  })
    ...
    chai.request(server)
      ...
      .end((err, res) => {
        ...
        chai.request(server)
          ...
          .end((err, res) => {
            assert.deepInclude(res.body, { result: 'successfully deleted' })
          })
